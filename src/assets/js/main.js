!(function (e) {
  ...
  t.on("load", function () {
    ...
    e(o).delay("500").fadeOut(2e3),
      a.on(
        "click",
        ".side-menu-wrap .side-menu-ul>li>a .btn-toggle",
        function () {
          return (
            e(this)
              .closest("li")
      ...
      a.on("click", ".humburger-menu .side-menu-close", function () {
        e(
          ".side-nav-container, .dashboard-nav-container, .user-nav-container"
        ).removeClass("active");
      }),
      e(i).each(function () {
        e(this)
          .find(".skill-item")
          .animate({ width: e(this).attr("data-percent") }, 3e3);
      }),
      t.on("scroll", function () {
        t.scrollTop() > 0
          ? e(s).addClass("header-fixed")
          : e(s).removeClass("header-fixed"),
          t.scrollTop() > 500
            ? e(r).addClass("btn-active")
            : e(r).removeClass("btn-active");
      }),
      ...
      e(g).length && e(g).magnificPopup({ type: "video" }),
      e(m).length &&
        e(m).daterangepicker({ opens: "right", singleDatePicker: !0 }),
      ...
      e(C).length &&
        e(C).jqte({
          placeholder:
            "Briefly describe about your company where you worked before",
          formats: [
            ["p", "Paragraph"],
            ["h1", "Heading 1"],
            ["h2", "Heading 2"],
      ...
      e(k).length && e(k).emojioneArea({ pickerPosition: "top" });
    var x,
      T = document.querySelector("#send-message-btn"),
      P = e(".contact-form"),
      S = e(".alert-message");
    function j(e) {
      (T.innerHTML = "Send Message"),
        S.fadeIn().removeClass("alert-danger").addClass("alert-success"),
        S.text(e),
        setTimeout(function () {
          S.fadeOut();
        }, 3e3),
        P.find('input:not([type="submit"]), textarea').val("");
    }
    function M(e) {
      (T.innerHTML = "Send Message"),
        S.fadeIn().removeClass("alert-success").addClass("alert-danger"),
    ...
    P.submit(function (t) {
      t.preventDefault(),
        ...
        (T.innerHTML = "Sending..."),
        setTimeout(function () {
          e.ajax({ type: "POST", url: P.attr("action"), data: x })
            .done(j)
            .fail(M);
        }, 2e3);
    }),
